...
import { CheckIcon, CloseIcon } from '@chakra-ui/icons';
  ...
  const MultichoiceTool = ({
    ...
    >
        ...
        <Heading size={'md'} paddingTop={'20px'}>Answer the question choosing between the given choices</Heading><br/>
        ...
        <Flex paddingTop={'20px'}>
            <CheckboxGroup onChange={handleChange} isDisabled={disable}>
                <Stack>
                    {data.choices.map(
                        (choice, index)=>{return (<>
                        <Checkbox value={choice}> {choice} 
                            <Box paddingLeft='10px' float='right' hidden={!disable}> 
                                <Icon as={(data.isChoiceCorrect[index]) ? CheckIcon : CloseIcon} color={(data.isChoiceCorrect[index]) ? 'green' : 'red'}/>
                            </Box>
                        </Checkbox></>)})
                    }
                </Stack>
        ...
        onClick={()=>{
            ...
            const edgesId = actualActivity?.validation.map((edge)=>{
              console.log(!checkBoxValue.map((value)=>data.choices.findIndex((choice)=>choice==value)).map((id)=>data.isChoiceCorrect[id]).filter((value)=>!value));
                if(!checkBoxValue.map((value)=>data.choices.findIndex((choice)=>choice==value)).map((id)=>data.isChoiceCorrect[id]).filter((value)=>!value) && edge.data.conditionKind=='pass')
                    return edge.id;
