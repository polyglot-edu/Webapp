...
const menu = urlParams.get("menu"); //menu parameter
    ...
    <div className="General">
      ...
      {currentPage === "startQuiz" && (
        <StartQuiz
          ...
          /*restartQuiz={restartQuiz}*/ question={question}
          quantityAnswer={quantityAnswer}
          tipologyAnswer={tipologyAnswer}
          choice={choice}
        />
      )}
...
function StartQuiz({
  ...
  const isCorrectButtonDisabled =
    localStorage.getItem("correctButtonDisabled") === "true";
  ...
  for (let i = 0; i < quantityAnswer; i += 2) {
    ...
    lineButtons.push(
      <button
        ...
        className={`w${i + 2}`}
        id={`w${i + 2}`}
        onClick={() => buttonClickHandler(i + 1)}
      >
        {/* Button content */} {option2}
      </button>,
    );
    lines.push(
  ...
  return (
    <div className="start">
      ...
      <div className="second_line">
        ...
        <div className="startbutton">{lines}</div>
      </div>
      {/*<button className='res' id="res" onClick={restartQuiz}>Restart Quiz To Developer</button> */}
      <div className="third_line">
        <button className="save" id="save" onClick={exit}>
          Save
        </button>
        <button
          className="question2"
          id="question2"
          onClick={() => nextQuiz(ctx, setCurrentPage)}
        >
          Next Activity
        </button>
      </div>
...
function NextVs(/*goBackToQuiz7*/) {
  return (
    <div className="Vscode">
      ...
      <div className="second_line">
        <h1 className="h1vs">
          The next page is a Code exercise, you need to turn back on Vscode and
          reload the second run line to do the Code exercise
        </h1>
      </div>
    </div>
  );
